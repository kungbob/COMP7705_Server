...
from app.ml.dataset import ImageFolder
from torchvision import transforms
from torch.utils.data import DataLoader, Dataset
from app.ml.networks import *
...
class UGATIT(object) :
    ...
    def build_model(self):
        ...
        self.trainA = ImageFolder(os.path.join('app/ml/dataset', self.dataset, 'trainA'), self.train_transform)
        self.trainB = ImageFolder(os.path.join('app/ml/dataset', self.dataset, 'trainB'), self.train_transform)
        self.testA = ImageFolder(os.path.join('app/ml/dataset', self.dataset, 'testA'), self.test_transform)
        self.testB = ImageFolder(os.path.join('app/ml/dataset', self.dataset, 'testB'), self.test_transform)
    ...
    def generateAnime(self, newlocation, newfilename):
        model_list = glob(os.path.join(self.result_dir, self.dataset, 'model', '*.pt'))
        if not len(model_list) == 0:
            model_list.sort()
            iter = int(model_list[-1].split('_')[-1].split('.')[0])
        ...
        '''
        real_A = Image.open(newlocation)
        real_A = transforms.functional.to_tensor(real_A)
        real_A.unsqueeze_(0)
        real_A = real_A.to(self.device)
        print(real_A)
        ...
        cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'test', 'A2B_%d.png' % newfilename), A2B * 255.0)
        '''
        ...
        for n, (real_A, _) in enumerate(newLoader):
            print(real_A)
            real_A = real_A.to(self.device)
            print(real_A)
            fake_A2B, _, fake_A2B_heatmap = self.genA2B(real_A)
